...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    (boundaries.minX = Math.min(boundaries.minX, body.x), boundaries.maxX = Math.max(boundaries.maxX, body.x))
    (boundaries.minY = Math.min(boundaries.minY, body.y), boundaries.maxY = Math.max(boundaries.maxY, body.y))
    boundaries
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val boundaries = new Boundaries
    (boundaries.minX = Math.min(a.minX, b.minX), boundaries.maxX = Math.max(a.maxX, b.maxX))
    (boundaries.minY = Math.min(a.minY, b.minY), boundaries.maxY = Math.max(a.maxY, b.maxY))
    boundaries
  }
  ...
  def computeSectorMatrix(bodies: coll.Seq[Body], boundaries: Boundaries): SectorMatrix =
    timeStats.timed("matrix") {
      ...
      parBodies.aggregate(sectorMatrix)(
        (sm: SectorMatrix, body: Body) => sm += body,
        (sm1: SectorMatrix, sm2: SectorMatrix) => sm1.combine(sm2)
      )
    }
  ...
  def eliminateOutliers(bodies: coll.Seq[Body],
